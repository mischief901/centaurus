defmodule CentaurusClient do
  ...
  test "Connect Der", context do
    ...
    {:ok, _socket} = Core.connect(socket_config, stream_config, 9001, {127,0,0,1}, %Options{}, 5_000)
    assert(true)
  end
  ...
  test "Connect Pem", context do
    ...
    stream_config = %StreamConfig{context[:stream_config_uni] | stream_pid: self()}
    {:error, _error} = Core.connect(socket_config, stream_config, 0, {0,0,0,0}, %Options{}, 5_000)
    assert(true)
